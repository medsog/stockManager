...
CREATE TABLE IF NOT EXISTS `m_reglfacture` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `id_facture` int(11) DEFAULT '0',
  `id_reglement` int(11) DEFAULT '0',
  `montant` float DEFAULT '0',
  `date_imputation` varchar(50) DEFAULT '0',
  PRIMARY KEY (`id`),
  KEY `FK_m_reglfacture_m_reglement` (`id_reglement`),
  KEY `FK_m_reglfacture_m_enteteventes` (`id_facture`),
  CONSTRAINT `FK_m_reglfacture_m_enteteventes` FOREIGN KEY (`id_facture`) REFERENCES `m_enteteventes` (`id`),
...
-- Export de données de la table gm.s_affaire : ~0 rows (environ)
